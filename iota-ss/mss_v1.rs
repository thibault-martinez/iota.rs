...
impl<S, G> crate::PrivateKeyGenerator for MssV1PrivateKeyGenerator<S, G>
    ...
    fn generate(&self, seed: &[i8], index: usize) -> Self::PrivateKey {
        ...
        for key_index in 0..(1 << (self.depth - 1)) {
            ...
            println!("{:?} - {:?} - {:?}", key_index, tree_index, ots_public_key.to_bytes().trytes());
            keys.push(ots_private_key);
            tree[tree_index * 243.. (tree_index + 1) * 243].copy_from_slice(ots_public_key.to_bytes());
        }
        ...
        for depth in (0..self.depth - 1).rev() {
            ...
            for i in 0..(1 << depth) {
                ...
                sponge.absorb(&tree[left_index * 243.. (left_index + 1) * 243]).unwrap();
                sponge.absorb(&tree[right_index * 243.. (right_index + 1) * 243]).unwrap();
                sponge.squeeze(&mut tree[index * 243.. (index + 1) * 243]).unwrap();
                sponge.reset();
            }
        }
...
impl<S, K> crate::PrivateKey for MssV1PrivateKey<S, K>
    ...
    fn sign(&mut self, message: &[i8]) -> Self::Signature {
        ...
        while tree_index != 0 {
            ...
            println!("sibling {:?} {:?}", sibling_index, self.tree[sibling_index * 243..(sibling_index + 1) * 243].to_vec().trytes());
            state[ots_signature.size() + i * 243.. ots_signature.size() + (i + 1) * 243].copy_from_slice(&self.tree[sibling_index * 243..(sibling_index + 1) * 243]);
            i = i + 1;
        }
...
mod tests {
    ...
    fn mss_v1_gen_test() {
        ...
        let wots_v1_kerl_private_key_generator = WotsV1PrivateKeyGeneratorBuilder::<Kerl>::default().security_level(SECURITY_LEVEL).build();
        // todo try with not recover
        let mss_v1_private_key_generator = MssV1PrivateKeyGeneratorBuilder::<Kerl, WotsV1PrivateKeyGenerator<Kerl>>::default().depth(DEPTH).generator(wots_v1_kerl_private_key_generator).build();
